...
class ActNorm(nn.Module):
    ...
    def forward(self, input, logdet=0, reverse=False):
        dimentions = input.size(2) * input.size(3)  # CHW,HW
        if reverse == False:
            input = input + self.bias
            input = input * torch.exp(self.logs)
            dlogdet = torch.sum(self.logs) * dimentions  # 计算对数行列式（Log Determinant）的增量
