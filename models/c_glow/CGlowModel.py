...
class CondGlowStep(nn.Module):
    ...
    def __init__(self, x_size, y_size, x_hidden_channels, x_hidden_size, y_hidden_channels):
        ...
        # 1. cond-actnorm
        self.actnorm = CondActNorm(x_size=x_size, y_channels=y_size[0], x_hidden_channels=x_hidden_channels,
                                           x_hidden_size=x_hidden_size)
        # 2. cond-1x1conv
        self.invconv = Cond1x1Conv(x_size=x_size, x_hidden_channels=x_hidden_channels,
...
class CondGlowModel(nn.Module):
    ...
    def sample(self, x, eps_std=1.0):
        mean, logs = self.prior()
        y = GaussianDiag.batchsampleR(x.size(0), mean, logs, eps_std)
        objective = GaussianDiag.logp(mean, logs, y)
        # 逆向变化，将y变成x，所以这里计算了一个先验分布采样得到了y
        y, logdet = self.flow(x, y, eps_std=eps_std, reverse=True)
        dimensions = y.size(1) * y.size(2) * y.size(3)
    ...
    def decode(self, x, z=None, eps_std=1.0, return_prob=False, zs=None, no_norm=False):
        mean, logs = self.prior()
        mean = mean if z is None else z
        y = GaussianDiag.batchsampleR(x.size(0), mean, logs, eps_std)
        objective = GaussianDiag.logp(mean, logs, y)
